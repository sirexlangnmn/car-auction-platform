                ...
                <div class="auction-item-2">
                    ...
                    <div class="auction-content">
                        ...
                        <div class="text-center">
                            <a href="#0" class="custom-button" onclick="submitBid('${current_bid}', '${uuid}', '${userUuid}', '${expiryDate}', 'amount${carId}')">Submit a bid</a>
...
function submitBid(current_bid, uuid, userUuid, expiryDate, amountId) {
    const availability = checkExpiredListing(expiryDate);
    console.log('availability : ', availability)
    const ownership = isItCarOfCurrentUser(userUuid)

    if (ownership === 'yes') {
        Swal.fire({
            title: 'Warning',
            text: "You can't bid on your owned car.",
            icon: 'warning',
            confirmButtonText: 'OK'
        })
    }

    if (ownership === 'no' && availability === 'active') {
        let result = bidProcess(current_bid, uuid);
        Swal.fire({
            title: result.title,
            text: result.message,
            icon: result.icon,
        ...
        if (result.message === 'Bid Success') {
            document.getElementById(amountId).innerHTML = result.currentBid;
        }
        displayListOfAuctions2();
    }

    if (ownership === 'no' && availability === 'expired') {
        let result = bidProcessForEpired(current_bid, uuid)
        Swal.fire({
            title: result.title,
            text: result.message,
            icon: result.icon,
...
function isItCarOfCurrentUser(userUuid) {
    let value;
    $.ajax({
        url: host + '/api/get/check-ownership',
        type: 'POST',
        data: {
            userUuid: userUuid,
        },
...
function bidProcess(current_bid, uuid) {
    let value;
    $.ajax({
        url: host + '/api/get/bid-process',
        type: 'POST',
        data: {
...
function bidProcessForEpired(current_bid, uuid) {
    let value;
    $.ajax({
        url: host + '/api/get/bid-process-for-expired',
        type: 'POST',
        data: {
            current_bid: current_bid,
            uuid: uuid,
...
function checkExpiredListing(expiryDate) {
    // Subtract 1 day's worth of milliseconds from the expiryDate
    var oneDayMilliseconds = 24 * 60 * 60 * 1000;
    var adjustedExpiryDate = new Date(new Date(expiryDate) - oneDayMilliseconds);

    // Get the current date
    let currentDate = new Date();

    // Compare the adjusted expiry date with the current date
    if (adjustedExpiryDate < currentDate) {
        return "expired";
    } else {
        // Calculate the time difference in milliseconds
        let timeDifference = adjustedExpiryDate - currentDate;
